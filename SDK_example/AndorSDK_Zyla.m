...
% readout
[~,currentIndex] = AT_GetEnumIndex(hndl,'PixelReadoutRate')                 % 0-10 MHz, 1-100 MHz, 2-200 MHz, 3-270 MHz
...
while(ishandle(h))
    ...
    [~,buf] = AT_WaitBuffer(hndl,1000);
    [~,buf2] = AT_ConvertMono16ToMatrix(buf,height,width,stride);
    figure(1), cla(gca), imshow(flip(transpose(buf2)),[])
    drawnow;
end
disp('Done.');
AT_Command(hndl,'AcquisitionStop');
